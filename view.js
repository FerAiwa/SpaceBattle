export { reload, createPlayerCards, updatePlayerCards }
...
function updatePlayerCards(players) {
  ...
  for (let i in players) {
    ...
    playerCard.querySelectorAll('span')
      .forEach((x, j) => x.innerText = countShipTypes(players[i], j))


    //Update portraits
    if (players[i].isDefeated) {
      playerCard.querySelector('figure img').style.filter = "grayscale(100%)"
    }
  }
}
